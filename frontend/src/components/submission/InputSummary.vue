...
<template>
  <aside class="input-summary" aria-labelledby="input-summary-heading">
    ...
    <dl><div><dt>Records</dt><dd :class="{ bad: !recordsOk }">{{ inspection.recordCount.toLocaleString() }}</dd></div><div><dt>Nucleotides</dt><dd :class="{ bad: !basesOk || !recordBasesOk }">{{ readableBases(inspection.baseCount) }}</dd></div><div><dt>Gene mode</dt><dd>{{ geneMode === 'meta' ? 'Meta' : 'Auto' }}</dd></div></dl>
    <div :class="['readiness-state', inspection.valid && recordsOk && basesOk && recordBasesOk && requestOk ? 'ready' : 'waiting']"><AppIcon :name="inspection.valid && recordsOk && basesOk && recordBasesOk && requestOk ? 'check' : 'info'" :size="17"/><span><strong>{{ inspection.valid && recordsOk && basesOk && recordBasesOk && requestOk ? 'FASTA is ready' : 'Input needs attention' }}</strong><small>{{ service.state === 'online' ? 'Ready for analysis' : 'Sample exploration remains available offline' }}</small></span></div>
  </aside>
</template>
