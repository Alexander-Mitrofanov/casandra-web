...
import { TERMINAL_STATUSES, phaseIndex, phasesForAnalysis } from "../../science.js";
...
const explicitAnalysisMode = computed(() => props.job?.options?.analysis_mode || props.job?.summary?.analysis_mode || "");
const analysisMode = computed(() => explicitAnalysisMode.value || "complete_genome");
const includeCrisprArrays = computed(() => {
  if (Object.hasOwn(props.job?.options || {}, "include_crispr_arrays")) return Boolean(props.job.options.include_crispr_arrays);
  if (Object.hasOwn(props.job?.summary || {}, "include_crispr_arrays")) return Boolean(props.job.summary.include_crispr_arrays);
  return !explicitAnalysisMode.value;
});
const progressPhases = computed(() => phasesForAnalysis(analysisMode.value, includeCrisprArrays.value).slice(0, -1));
const currentIndex = computed(() => {
  const current = props.job?.phase || (props.job?.status === "completed" ? "completed" : "queued");
  const exact = progressPhases.value.findIndex((phase) => phase.id === current);
  if (exact >= 0) return exact;
  const currentRank = phaseIndex(current);
  return progressPhases.value.reduce((index, phase, candidate) => phaseIndex(phase.id) <= currentRank ? candidate : index, 0);
});
