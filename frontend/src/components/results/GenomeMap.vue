...
watch(currentFeatureRows, (next) => {
  if (selectedFeatureKey.value && !next.some(([row, kind]) => featureKey(row, kind) === selectedFeatureKey.value)) {
    selectedFeatureKey.value = "";
    emit("feature-selected", null);
  }
});
...
<template>
  <section class="result-section genome-map" :aria-labelledby="headingId">
    <div class="result-heading"><div><p class="eyebrow">Coordinate explorer</p><h3 :id="headingId">Source-forward feature map</h3></div><p>Left is base 1 in the submitted record. Arrowheads encode gene strand; the contig is never reverse-complemented for display.</p></div>
    ...
    <div v-if="contigs.length" class="map-scroll" tabindex="0" aria-label="Scrollable genomic feature plot">
      ...
      <svg viewBox="0 0 1000 278" role="group" :aria-labelledby="`${mapTitleId} ${mapDescriptionId}`">
        ...
        <defs><pattern :id="arrayPatternId" width="8" height="8" patternUnits="userSpaceOnUse"><rect width="5" height="8" fill="#a873e7"/><rect x="5" width="3" height="8" fill="#e9ddfb"/></pattern></defs>
