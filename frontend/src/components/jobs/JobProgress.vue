...
<template>
  <section :class="['job-panel', `job-${job?.status || 'queued'}`]" aria-labelledby="job-heading">
    <div class="job-heading"><div><p class="eyebrow">Private analysis job</p><h2 id="job-heading" tabindex="-1">{{ statusTitle }}</h2><p class="job-id">Job <code>{{ credential.jobId }}</code><span>·</span>{{ job?.expires_at || credential.expiresAt ? `expires ${formatDate(job?.expires_at || credential.expiresAt)}` : 'retention begins after completion' }}</p></div><span :class="['job-badge', successful ? 'success' : terminal ? 'terminal' : 'active']" role="status"><i/>{{ successful ? 'Results ready' : terminal ? statusTitle : job?.queue_position ? `Queue ${job.queue_position}` : 'In progress' }}</span></div>
    ...
    <div v-if="terminal" class="queue-row"><span>Download needed artifacts before the retention window closes.</span><button class="leave-job-button" type="button" @click="$emit('forget')">Leave job and start another</button></div>
  </section>
</template>
