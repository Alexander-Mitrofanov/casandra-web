...
<template>
  <section v-if="decisions" class="result-section specificity-evidence" aria-labelledby="specificity-heading">
    ...
    <template v-if="withheld.length">
      ...
      <div class="specificity-table-wrap"><table aria-label="Withheld genomic proteins"><thead><tr><th>Protein and source</th><th>Position</th><th>Decision</th><th>Original-core family</th><th>Reason</th></tr></thead><tbody><tr v-for="row in visible" :key="row.protein_id"><th scope="row"><code>{{ row.protein_id }}</code><br>{{ row.contig_id }}</th><td>{{ row.start }}–{{ row.end }} ({{ row.strand }})</td><td>{{ decisionLabel(row) }}</td><td>{{ row.core_original_prediction?.cas_family || 'Not reported' }}</td><td>{{ reasonLabel(row) }}<details><summary>Original evidence and rule details</summary><pre>{{ JSON.stringify({ core_original_prediction: row.core_original_prediction, repair_evidence: row.repair_evidence }, null, 2) }}</pre></details></td></tr></tbody></table></div>
      <div v-if="withheld.length > 20" class="result-pagination"><button :disabled="page === 0" @click="page--">Previous withheld proteins</button><span>Page {{ page + 1 }} of {{ Math.ceil(withheld.length / 20) }}</span><button :disabled="(page + 1) * 20 >= withheld.length" @click="page++">Next withheld proteins</button></div>
