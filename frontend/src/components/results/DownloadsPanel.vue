...
<template>
  <section id="result-downloads" class="result-section downloads" aria-labelledby="downloads-heading"><div class="result-heading"><div><h3 id="downloads-heading">Download results</h3></div><p>Save complete annotations as JSON, open tables as CSV, or use FASTA sequences in your next analysis.</p></div>
    ...
    <template v-else>
      <div class="preferred-downloads">
        <p class="download-format-note"><strong v-for="format in availableFormats" :key="format">{{ formatLabel(format) }}</strong><span>Complete, untruncated results</span></p>
